...
public class WhiteEyeController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if(adamFound)
        {
            ...
            print(Vector3.Distance(transform.position, adam.transform.position));
            //navmesh chaseplayer
            if(Vector3.Distance(transform.position, adam.transform.position) > 10)
            {
                Patrol();
            }
            else if(Vector3.Distance(transform.position, adam.transform.position) < 3)
            {
                Attack();
            }
            else ChasePlayer();
    ...
    private void SearchForDes()
    {
        ...
        desPoint = new Vector3(transform.position.x + x, 1, transform.position.z +z);
